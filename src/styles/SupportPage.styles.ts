...
export const HeroImage = styled.img`
  ...
  object-fit: contain;
  max-height: 500px;
  filter: drop-shadow(0 20px 50px rgba(249, 115, 22, 0.4));
  border-radius: 20px;
...
export const FAQQuestion = styled.button<{ $isOpen: boolean }>`
  ...
  background: ${props => props.$isOpen ? 'linear-gradient(135deg, #FFF7ED 0%, #FFEDD5 100%)' : '#ffffff'};
  border: none;
  text-align: left;
  cursor: pointer;
  display: flex;
  ...
  font-size: 1.15rem;
  font-weight: 700;
  color: ${props => props.$isOpen ? '#F97316' : '#9A3412'};
  transition: all 0.3s ease;
  position: relative;
...
export const FAQIcon = styled.div<{ $isOpen: boolean }>`
  ...
  transition: all 0.3s ease;
  flex-shrink: 0;
  background: ${props => props.$isOpen ? 'linear-gradient(135deg, #F97316, #FB923C)' : 'transparent'};
  border-radius: 50%;
  padding: 4px;
  ...
  svg {
    width: 100%;
    height: 100%;
    color: ${props => props.$isOpen ? '#ffffff' : '#F97316'};
